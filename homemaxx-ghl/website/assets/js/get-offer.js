...
(function () {
  ...
  async function updateSlotsDisplay() {
    try {
      const res = await fetch('YOUR_GHL_SLOTS_WEBHOOK_URL', { headers: { 'Accept': 'application/json' }, cache: 'no-store' });
  ...
  function createOpendoorFlow() {
    ...
    function showContactInfo() {
      ...
      const step3 = steps[2];
      step3.innerHTML = createContactInfoStep();
    }
    
    // Enhanced contact info step with Google login integration
    function createContactInfoStep() {
      return `
        <div class="step-content">
          <div class="step-header">
            <h2>Get your cash offer</h2>
            <p class="step-description">We'll send your offer details to the contact information below.</p>
          </div>
          ...
          <div class="form-actions">
            <button type="submit" class="btn btn-primary btn-large" id="submitOffer">
              Get My Instant Cash Offer
            </button>
          </div>
          
          <div class="legal-notice">
            <p>By continuing, you agree to our <a href="pages/legal/terms-of-service.html" target="_blank">Terms of Service</a> and <a href="pages/legal/privacy-policy.html" target="_blank">Privacy Policy</a>.</p>
            <p>You consent to receive calls and texts, including automated messages.</p>
          </div>
        </div>
      `;
    }
  ...
  function attachPlaces() {
    try {
      ...
      async function propertyLookup(address) {
        try {
          const url = `YOUR_GHL_PROPERTY_LOOKUP_WEBHOOK_URL?address=${encodeURIComponent(address)}`;
  ...
  document.addEventListener('ghl:submit:success', function () {
    ...
    try {
      ...
      fetch('YOUR_GHL_SLOTS_WEBHOOK_URL', {
        method: 'POST',
  ...
  // Enhanced initialization with resume functionality
  function initializeFunnel() {
    ...
    if (isResuming && resumeStep) {
      current = parseInt(resumeStep);
      console.log(`Resuming funnel at step ${current}`);
    }
    ...
    // Show the correct step (either first or resumed step)
    showStep(current);
    ...
    if (addressParam && window.offerResume) {
      // Pre-fill address input if resuming
      const addressInput = document.getElementById('propertyAddress');
      if (addressInput) {
        addressInput.value = addressParam;
        // Trigger property lookup for resumed address
        if (window.propertyLookup) {
          propertyLookup(addressParam);
        }
      }
